...
pub const ELF64_EHDR_SIZE : usize = size_of::<Elf64Ehdr>();
...
pub fn make_elf64(code: &[u8], relocs: Vec<Relocation>, symbols: &[Symbol], outpath: &PathBuf) -> Vec<u8>{
    // elf class
    // 1 - 32-bit object
    // 2 - 64-bit object
    let ei_class = 2;
        ...
        for (index, symbol) in symbols.iter().enumerate(){
            ...
            if symbol.visibility == Visibility::Global{
                ...
                elf_symbs.push(Elf64Sym{
                    name: strtab.len() as Elf64Word,
                    value: symbol.offset,
                    size: if let Some(s) = symbol.size {s as u64} else {0},
    ...
    let mut symbols_offset = 
        (size_of::<Elf64Ehdr>() + code.len() + (elf_header.shnum as usize * size_of::<Elf64Shdr>())) as Elf64Off;
    ...
    strtab_hdr.offset = symbols_offset + (symbols.len() * size_of::<Elf64Sym>()) as u64 + shstrtab.len() as u64;
    ...
    if let Some(mut shdr) = rel_text_shdr{
        ...
        shdr.offset = symbols_offset + (symbols.len() * size_of::<Elf64Sym>()) as u64 + shstrtab.len() as u64 + 
            strtab.len() as u64;
    ...
    if let Some(mut shdr) = rela_text_shdr{
        ...
        // what is this
        shdr.offset = symbols_offset + (symbols.len() * size_of::<Elf64Sym>()) as u64 + shstrtab.len() as u64 + 
            strtab.len() as u64 + (rel_text_symb.len() * size_of::<Elf64Rel>()) as u64;
        bytes.extend(shdr.bytes());
    }
